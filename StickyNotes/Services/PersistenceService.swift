...
class PersistenceService: ObservableObject {
    ...
    func saveNote(_ note: Note) async throws {
        ...
        // Do NOT call loadNotes() — callers manage their own state.
        // Calling loadNotes() after every save causes full re-read storms.
    }
    ...
    func createBackup() async throws -> URL {
        // Name with timestamp; use directory copy (not zip — zip requires external library)
        let backupName = "StickyNotes_Backup_\(Int(Date().timeIntervalSince1970))"
        let backupURL = FileManager.default.temporaryDirectory.appendingPathComponent(backupName)

        // Remove existing backup at this path if any
