...
class SearchIndexingOperation: Operation {
    ...
    override func main() {
        ...
        // Use a semaphore to bridge async Core Data calls into synchronous Operation.main()
...
class ExportOperation: Operation {
    ...
    override func main() {
        ...
        do {
            ...
            // Export each note using a semaphore to bridge async into sync Operation.main()
            let semaphore = DispatchSemaphore(value: 0)
            ...
            let capturedIsCancelled = { self.isCancelled }

            Task {
                do {
                    let service = CoreDataPersistenceService.shared
                    for note in capturedNotes {
                        guard !capturedIsCancelled() else { break }
                        let noteExportURL = try await service.exportNote(note, format: capturedFormat)
                        let fileName = "\(note.displayTitle).\(capturedFormat.fileExtension)"
                        let finalURL = exportDir.appendingPathComponent(fileName)
                        try FileManager.default.moveItem(at: noteExportURL, to: finalURL)
                    }

                    let zipDest = tempDir.appendingPathComponent("StickyNotes_Export.zip")
                    try self.createZipArchive(from: exportDir, to: zipDest)
