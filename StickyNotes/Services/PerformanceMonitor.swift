...
class PerformanceMonitor {
    ...
    // Capped at 120 samples (1 hour at 30s intervals) to prevent unbounded growth
    private let maxMemorySamples = 120
    ...
    private func recordMemoryUsage() {
        ...
        if kerr == KERN_SUCCESS {
            let sample = info.resident_size
            monitorQueue.async {
                self.memoryUsage[Date()] = sample
                // Prune oldest samples to prevent unbounded growth
                if self.memoryUsage.count > self.maxMemorySamples {
                    let sorted = self.memoryUsage.keys.sorted()
                    let toRemove = sorted.prefix(self.memoryUsage.count - self.maxMemorySamples)
                    toRemove.forEach { self.memoryUsage.removeValue(forKey: $0) }
                }
            }
        }
    }
    ...
    private func benchmarkCoreDataOperations() async {
        // IMPORTANT: Benchmarks must use an in-memory store, NOT the shared production store.
        // Writing test data to CoreDataPersistenceService.shared pollutes user data.
        // This benchmark is disabled pending a proper in-memory test harness.
        print("  Core Data benchmark: disabled (requires in-memory store to avoid polluting user data)")
    }
