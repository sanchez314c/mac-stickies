...
extension CGPoint {
    ...
    /// String representation of the point (matches CGPoint(string:) parser format)
...
extension CGSize {
    ...
    /// String representation of the size (matches CGSize(string:) parser format)
    var stringValue: String {
        "\(width), \(height)"
    }
}
...
extension Note {
    ...
    func hasAnyTag(_ tags: [String]) -> Bool {
        !Set(self.tags).isDisjoint(with: Set(tags))
    }
