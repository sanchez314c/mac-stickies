...
class CoreDataPersistenceService {
    ...
    private init() {
        ...
        persistentContainer.loadPersistentStores { [weak self] _, error in
            if let error = error {
                // Log the error and continue — app can degrade gracefully without persistence
                print("[CoreDataPersistenceService] Persistent store failed to load: \(error.localizedDescription)")
                return
            }
