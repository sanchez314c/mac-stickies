...
public final class PersistenceController {
    ...
    init(inMemory: Bool = false) {
        ...
        guard let _ = model.entities.first else {
            ...
            // Continue — model may still function with dynamic entities
    ...
    private func loadPersistentStores() {
        ...
        container.loadPersistentStores { [weak self] description, error in
            if let error = error {
                ...
                print("[PersistenceController] Failed to load persistent stores: \(error.localizedDescription)")
                self?.errorPublisher.send(.storeLoadFailed(error))
            }
    ...
    private func attemptStoreRecovery(for description: NSPersistentStoreDescription, error: Error) {
        guard let storeURL = description.url else {
            print("[PersistenceController] Cannot recover store without URL: \(error.localizedDescription)")
            errorPublisher.send(.storeLoadFailed(error))
            return
        }
        ...
        do {
            ...
            container.loadPersistentStores { [weak self] description, error in
                if let error = error {
                    print("[PersistenceController] Store recovery failed: \(error.localizedDescription)")
                    self?.errorPublisher.send(.storeLoadFailed(error))
                    return
                }
